...
async def reset_dns():
    ...
    try:
        ...
        raise Exception(f"Failed to reset DNS. Command: {cmd}. Error: {e.stderr.strip()}")
    except Exception as e:
        error_detail = str(e) if str(e) else "Unknown error"
        raise Exception(f"Failed to initiate subprocess for command {cmd}. Error: {error_detail}")