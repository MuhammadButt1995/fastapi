...
def extract_key_value_pairs(raw_output: str) -> Dict[str, str]:
    ...
    return {key.strip(): value.strip() for line in lines if ": " in line for key, value in [line.split(": ", 1)]}

def determine_signal_quality(value: int, thresholds: Dict[str, int], metric_type: str) -> str:
...
def compute_overall_quality(scores: Dict[str, str]) -> str:
    ...
    overall_score = sum(QUALITY_SCORES[scores[key]] * weight for key, weight in weights.items() if key in scores)
...
def fetch_windows_wifi_data() -> Dict[str, Any]:
    ...
    try:
        ...
        scores = {
            "signal": determine_signal_quality(signal_strength, SIGNAL_QUALITY_THRESHOLDS, "percentage"),
            "radio_type": next((quality for quality, types in RADIO_TYPE_THRESHOLDS.items() if radio_type_string in types), "slow"),
...
def fetch_macos_wifi_data() -> Dict[str, Any]:
    ...
    try:
        ...
        scores = {
            "signal": determine_signal_quality(signal_strength, SIGNAL_QUALITY_THRESHOLDS_DBM, "dBm"),
        ...
        return {
            "signal": {"quality": scores["signal"], "value": signal_strength},
            "radio_type": {"quality": scores["radio_type"], "value": {"lastTxRate": last_tx_rate}},
            "channel": {"quality": scores["channel"], "value": channel},
            "overall": overall_quality.upper(),
            "description": QUALITY_DESCRIPTIONS[overall_quality],
            "rating": QUALITY_RATINGS[overall_quality],
