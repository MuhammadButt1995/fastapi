    ...
    def get_LDAP_data(self):
        ...
        try:
            if platform.system() == 'Windows':
                ...
                last_logon = subprocess.check_output('net user %s /domain | findstr /C:"Last logon"' % user, shell=True).decode().strip()
                password_expires = subprocess.check_output('net user %s /domain | findstr /C:"Password expires"' % user, shell=True).decode().strip()
                password_expires_datetime = datetime.strptime(password_expires, '%m/%d/%Y %I:%M %p')
